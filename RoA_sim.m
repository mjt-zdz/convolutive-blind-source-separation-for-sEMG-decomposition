...
We assume one of the two sets of results be the gold standard and measure how close it is to the other set of results. 
RoA is calculated as below:

RoA = cj/(cj + Aj + Bj)

where cj is the number of discharges of the jth motor unit spike train that was identified by both decompositions (tolerance in discharge timing±0.5 ms), 
Aj is the number of discharges identified only by one of the two decompositions, 
and Bj is the number of discharges identified only by the other decomposition.
...
Outputs
    roa: If there are n number of motor units identified in the first set of results and 
    m number of motor units in the second set of results, roa will be an nbym matrix. E.G. 
    The element at fifth row and seventh column of the roa matrix is the result of comparing 
    the fifth identified motor unit in the first set of results to the seventh identified motor unit in the second set of results.
%}

roa = zeros(numel(MUPulses1), numel(MUPulses2));
cj = zeros(numel(MUPulses1), numel(MUPulses2));

% A, B and C are used with the same definition they have in the formula for RoA 
for ii = 1:numel(MUPulses1)
    for jj = 1:numel(MUPulses2)
        ...
        for k = 1: length(timeA)
            ...
            % calculate the range of time to look for a common firing in the other spike train
            low = timeA(k) - tol;
            high = timeA(k) + tol;
            
            % finds the indices of the common firing indices between A and B
